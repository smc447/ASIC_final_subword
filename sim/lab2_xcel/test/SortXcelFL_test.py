...
def gen_xcel_protocol_msgs( base_addr, size, stack_addr ):
  return [
    xreq( 'wr', 1, base_addr ), xresp( 'wr', 0 ),
    xreq( 'wr', 2, size      ), xresp( 'wr', 0 ),
    xreq( 'wr', 3, stack_addr), xresp( 'wr', 0 ),
...
# Test Cases
#-------------------------------------------------------------------------
rand_large_data = random.randint(0,0x7fffffff)
rand_small_data = random.randint(0,0xffff)

mini               = [ 0x21, 0x14, 0x42, 0x03 ]
same_small_data    = [ rand_small_data     for i in range(32) ]
same_large_data    = [ rand_large_data     for i in range(32) ]
...
nonpow2_size_large = [ random.randint(0,0x7fffffff) for i in range(67) ]
...
test_case_table = mk_test_case_table([
  ...
  [ "same_small_data",    same_small_data,0,  0,   0,    0   ],
  [ "same_large_data",    same_large_data,0,  0,   0,    0   ],
  ...
  [ "nonpow2_size_large", nonpow2_size_large,   0,  0,   0,    0   ],
...
def run_test( xcel, cmdline_opts, test_params ):
  ...
  xcel_protocol_msgs = gen_xcel_protocol_msgs( 0x1000, len(data), 0x2000 )
  ...
  mem_vals = th.mem.read_mem( 0x1000, len(data_bytes*2))
  mem_result = list(struct.unpack("<{}I".format(len(data*2)),mem_vals))
...
def run_test_multiple( xcel, cmdline_opts ):
  ...
  for i in range(8):
    msgs.extend( gen_xcel_protocol_msgs( base_addr+i*16, 4, stack_addr+i*16 ) )
...
def test( cmdline_opts, test_params ):
  run_test( SortXcelFL(), cmdline_opts, test_params )

def test_multiple( cmdline_opts ):
  run_test_multiple( SortXcelFL(), cmdline_opts )
