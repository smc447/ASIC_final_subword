...
#include "obj_dir_NullXcel_noparam/VNullXcel_noparam.h"
#include "stdio.h"
#include "stdint.h"
#include "verilated.h"
...
#if VLINETRACE
#include "obj_dir_NullXcel_noparam/VNullXcel_noparam__Syms.h"
#include "svdpi.h"
#endif
...
extern "C" {
  ...
  // Exposed methods
  VNullXcel_noparam_t * create_model( const char * );
  void destroy_model( VNullXcel_noparam_t *);
  void comb_eval( VNullXcel_noparam_t * );
  void seq_eval( VNullXcel_noparam_t * );
  void assert_en( bool en );

  #if VLINETRACE
  void trace( VNullXcel_noparam_t *, char * );
  #endif
...
VNullXcel_noparam_t * create_model( const char *vcd_filename ) {

  VNullXcel_noparam_t * m;
  VNullXcel_noparam   * model;

  Verilated::randReset( 0 );
  Verilated::randSeed( 0 );

  m     = (VNullXcel_noparam_t *) malloc( sizeof(VNullXcel_noparam_t) );
  model = new VNullXcel_noparam();
  ...
  m->clk = &model->clk;
  m->reset = &model->reset;
  m->mem_reqstream_msg = model->mem_reqstream_msg;
  m->mem_reqstream_rdy = &model->mem_reqstream_rdy;
  m->mem_reqstream_val = &model->mem_reqstream_val;
  m->mem_respstream_msg = &model->mem_respstream_msg;
  m->mem_respstream_rdy = &model->mem_respstream_rdy;
  m->mem_respstream_val = &model->mem_respstream_val;
...
void destroy_model( VNullXcel_noparam_t * m ) {

  #if VM_COVERAGE
    VerilatedCov::write( "coverage.dat" );
  #endif

  VNullXcel_noparam * model = (VNullXcel_noparam *) m->model;
...
void comb_eval( VNullXcel_noparam_t * m ) {
...
void seq_eval( VNullXcel_noparam_t * m ) {
...
#if VLINETRACE
void trace( VNullXcel_noparam_t * m, char* str ) {
  ...
  svSetScope( &model->__VlSymsp->__Vscope_NullXcel_noparam__v );
