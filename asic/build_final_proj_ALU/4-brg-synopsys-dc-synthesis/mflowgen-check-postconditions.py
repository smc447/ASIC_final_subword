...
def test_7_():
    assert 'ELAB-405' not in File( 'logs/dc.log' ), "Multiply driven nets found in the design"

def test_8_():
    assert 'ELAB-974' not in File( 'logs/dc.log' ), "Inferred latch(es) found inside an always_comb block in the design"

def test_9_():
    assert 'ELAB-978' not in File( 'logs/dc.log' ), "Inferred latch(es) found inside an always_ff block in the design"
...
def main():
  ...
  print()
  print( GREEN + '    > Checking postconditions for step "brg-synopsys-dc-synthesis"' + END )
  print()
